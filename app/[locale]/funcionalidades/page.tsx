...
// --- Mock UI components (receive translated labels as props) ---

function CRMMock({ statuses }: { statuses: string[] }) {
  const colors = ["#3b82f6", "#F7CC01", "#22c55e"];
  const clients = ["Hernán Rodríguez — Campaña Nike", "Laura Méndez — Spot TV", "Carlos Vega — Docu Norte"];
  return (
    <div className="p-4 space-y-2">
      {clients.map((client, i) => (
        <div
          ...
          style={{ background: "#1a1a1a", borderLeft: `3px solid ${colors[i]}` }}
        >
...
function KanbanMock({ columns }: { columns: string[] }) {
  const bgs = ["#1f2937", "#1e3a5f", "#3b1f5f"];
  const texts = ["#9ca3af", "#93c5fd", "#c4b5fd"];
  const cards = [["Campaña Adidas"], ["Videoclip Lali", "Spot BNA"], ["Docu Patagonia"]];
  return (
    <div className="p-4 flex gap-2">
      {columns.map((col, i) => (
        <div key={col} className="flex-1 flex flex-col gap-1.5 min-w-0">
          <div className="text-xs font-medium px-2 py-1 rounded truncate" style={{ background: bgs[i], color: texts[i] }}>
            {col}
          </div>
          {(cards[i] ?? []).map((card) => (
            <div
              key={card}
              className="text-xs px-2 py-2 rounded truncate"
          ...
          return (
            <div
              ...
              style={{
                ...
                color: day === 19 ? "#0A0A0A" : bg ? "#fff" : "#6b7280",
...
function CrewMock({ headers, exportBtn }: { headers: string[]; exportBtn: string }) {
  const crew = [
    { name: "Marcos Paz", rol: "Dir. de Foto", cuil: "20-31234567-4" },
    { name: "Ana García", rol: "Sonidista", cuil: "27-28765432-1" },
    { name: "Luis Torres", rol: "Gaffer", cuil: "20-24111222-8" },
  ];
  return (
    <div className="p-4">
      <div className="flex text-xs text-gray-600 border-b border-gray-800 pb-2 gap-3 mb-2">
        {headers.map((h) => (
          <span key={h} className={h === headers[0] ? "flex-1" : "w-20"}>{h}</span>
        ))}
      </div>
      <div className="space-y-2">
        {crew.map((c) => (
          <div key={c.name} className="flex text-xs gap-3 items-center">
            ...
            <span className="w-20 text-gray-500 truncate">{c.rol}</span>
            <span className="w-20 text-gray-600 font-mono">{c.cuil}</span>
          </div>
        ))}
      </div>
      <div className="mt-4">
        <span className="text-xs px-3 py-1.5 rounded cursor-default" style={{ background: "var(--brand)", color: "#0A0A0A" }}>
          {exportBtn}
        </span>
      </div>
    </div>
...
function ProveedoresMock() {
  const provs = [
    { name: "Luces Profesionales SA", cat: "Iluminación", costo: "$1.200 USD" },
    { name: "Dolly Rent LATAM", cat: "Cámara/Grip", costo: "$850 USD" },
    { name: "Catering El Rancho", cat: "Catering", costo: "$340 ARS×día" },
  ];
  return (
    <div className="p-4 space-y-2">
      {provs.map((p) => (
        ...
        >
          <div className="min-w-0 mr-4">
            <div className="text-gray-300 font-medium truncate">{p.name}</div>
            <div className="text-gray-600">{p.cat}</div>
          </div>
          <span className="font-mono text-xs flex-shrink-0" style={{ color: "var(--brand)" }}>{p.costo}</span>
        </div>
      ))}
    </div>
  );
...
function IAMock({ userMsg, aiMsg, typing }: { userMsg: string; aiMsg: string; typing: string }) {
  return (
    <div className="p-4 space-y-3">
      <div className="text-xs rounded-xl px-3 py-2.5 max-w-[80%]" style={{ background: "#1e3a5f", color: "#93c5fd" }}>
        {userMsg}
      </div>
      <div
        className="text-xs rounded-xl px-3 py-2.5 ml-auto max-w-[80%]"
        style={{ background: "#1a1a1a", border: "1px solid #2a2a2a", color: "#d1d5db" }}
      >
        {aiMsg}
      </div>
...
function buildMockContent(index: number, labels: ModuleMockLabels): ReactNode {
  switch (index) {
    case 0:
      return <CRMMock statuses={labels.statuses ?? []} />;
    case 1:
      return <KanbanMock columns={labels.columns ?? []} />;
    case 2:
      return <AgendaMock />;
    case 3:
      return <CrewMock headers={labels.headers ?? []} exportBtn={labels.exportBtn ?? ""} />;
    case 4:
      return <ProveedoresMock />;
    case 5:
      return (
        <IAMock
          userMsg={labels.userMsg ?? ""}
          aiMsg={labels.aiMsg ?? ""}
  ...
  return (
    <div style={{ background: "var(--bg)" }}>
      ...
      {modules.map((mod, i) => (
        <section
          key={mod.title}
          style={{ background: i % 2 === 0 ? "#111111" : "var(--bg)" }}
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid lg:grid-cols-2 gap-12 items-center">
              ...
              <div className={i % 2 !== 0 ? "lg:order-2" : ""}>
                ...
                <p className="mt-1 text-base font-semibold" style={{ color: "var(--brand)" }}>
                  {mod.headline}
                </p>
                <p className="mt-3 text-sm text-gray-500 italic leading-relaxed border-l-2 pl-3" style={{ borderColor: "#2a2a2a" }}>
                  {mod.problem}
                </p>
                ...
                <Link
                  href="/registro"
                  className="mt-6 inline-flex items-center text-sm font-semibold transition-opacity hover:opacity-80"
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {comingSoonItems.map((item) => (
              <div
                key={item.title}
                className="rounded-xl p-6 border flex items-start gap-4 opacity-70"
